		...
		if vehicle and vehPlate then
			...
			if user_id == plateUserId or stealVehs[vehPlate] == user_id then
				...
				if vehLock == 1 then
					...
					TriggerClientEvent("Notify",source,"unlocked","Destrancado",5000)
				else
					TriggerClientEvent("vrp_sound:source",source,"lock",0.3)
					TriggerClientEvent("Notify",source,"locked","Trancado",5000)
				end
...
RegisterCommand("car",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
 	if user_id then
		if vRP.hasPermission(user_id,"Dev2021") and args[1] then
		...
		if args[1] == "transfer" and parseInt(args[3]) > 0 then
			...
			if myvehicles[1] then
				...
				if vRP.request(source,"Deseja transferir o veículo <b>"..vRP.vehicleName(tostring(args[2])).."</b> para <b>"..identity.name.."</b>?",30) then
					...
					if vehicle[1] then
						TriggerClientEvent("Notify",source,"vermelho","<b>"..identity.name.." "..identity.name2.."</b> já possui este modelo de veículo.",5000)
					else
			...
			local vehicle = vRP.query("vRP/get_vehicle",{ user_id = parseInt(user_id) })
			for k,v in ipairs(vehicle) do
				TriggerClientEvent("Notify",source,"amarelo","<b>Modelo:</b> "..vRP.vehicleName(v.vehicle).." ( "..v.vehicle.." )",20000)
				Citizen.Wait(1)
			end
		end
	end
