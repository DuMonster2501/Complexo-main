$(document).ready(() => {
  ...
  window.addEventListener('message',function(event){
      ...
      if (event.data.initialBarbershop) {
        $('.left-person-input').css('opacity', 1);
        $('.right-person-input .category-content.3').css('opacity', 1);
      } else {
        $('.left-person-input').css('opacity', 0);
        $('.right-person-input .category-content.3').css('opacity', 0);
      }
		}
  ...
  $('.left-person-content .category-item').on('click', function () {
    $('.left-person-content .category-item').removeClass('active');
    $('.left-person-content .category-content').removeClass('active');
    $(this).addClass('active');
    $('.left-person-content .category-content.'+$(this).attr('index')).addClass('active');
  });

  $('.right-person-content .category-item').on('click', function () {
    $('.right-person-content .category-item').removeClass('active');
    $('.right-person-content .category-content').removeClass('active');
    $(this).addClass('active');
    $('.right-person-content .category-content.'+$(this).attr('index')).addClass('active');
  });

  $('input[type="range"].slider-progress').on('input', function() {
    $(this).parent().find('.input-group-values').find('p:first-child').text($(this).val());
    change();
  });

  $('input[type="range"].slider-progress').on('change', function() {
    $(this).parent().find('.input-group-values').find('p:first-child').text($(this).val());
    change();
  });
  ...
  function changeValue() {
    for (let e of document.querySelectorAll('input[type="range"].slider-progress')) {
      ...
      $(e).parent().find('.input-group-values').find('p:first-child').text($(e).val());
    }
  }
