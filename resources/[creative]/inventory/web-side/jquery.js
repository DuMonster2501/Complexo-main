	...
	$(".populated").droppable({
		...
		drop: function(event,ui){
			...
			$(document).ready(function() {
				...
				$(document).keydown(function(e) {
					if (ctrlDown && (e.keyCode == cKey)) console.log("");
					if (ctrlDown && (e.keyCode == vKey)) console.log("");
				});
			});
...
const updateMochila = () => {
	$.post("http://inventory/requestMochila",JSON.stringify({}),(data) => {
		...
		if (lastHunger !== data["hunger"]){
			lastHunger = data["hunger"];
			console.log("")
			$("#displayHunger").css("width",data["hunger"] + "%");
		}
	
		if (lastWater !== data["thirst"]){
			lastWater = data["thirst"];
			console.log("")
			$("#displayThirst").css("width",data["thirst"] + "%");
		}
	
		if (lastStress !== data["stress"]){
			lastStress = data["stress"];
			console.log("")
			$("#displayStress").css("width",data["stress"] + "%");
		}
		...
		for (let x = 1; x <= data["maxpeso"]; x++){
			...
			if (data["inventario"][slot] !== undefined){
				...
				let checkDurabilidade
				if (!isNaN(actualPercent)){
					checkDurabilidade = actualPercent
					console.log("")
				} else {
					checkDurabilidade = 0
				}
				...
				const item = `<div class="item populated" title="" data-max="${v["max"]}" data-type="${v["type"]}" data-serial="${v["serial"]}" style="background-image: url('images/${v["index"]}.png'); background-position: center; background-repeat: no-repeat;" data-amount="${v["amount"]}" data-peso="${v["peso"]}" data-item-key="${v["key"]}" data-name-key="${v["name"]}" data-slot="${slot}" data-description="${v["desc"]}" data-economy="${v["economy"]}">
					...
					<div class="durability" style="width: ${checkDurabilidade == 1 ? "100":checkDurabilidade}%; background: ${actualPercent == 1 ? "#fc5858":colorPicker(actualPercent)};"></div>
