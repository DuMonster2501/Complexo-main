...
function tryTakeChips(source,amount)
	...
	local user_id = vRP.getUserId(thePlayer)
		if vRP.tryGetInventoryItem(user_id,"chips",amount,false) then
...
function giveChips(source,amount)
    local thePlayer = source
	local user_id = vRP.getUserId(thePlayer)
	vRP.giveInventoryItem(user_id,"chips",amount,false)
    updatePlayerChips(source)
end
            ...
            if tonumber(betAmount) then
                ...
                if betAmount > 0 then
                    if tryTakeChips(source,betAmount) then
                        ...
                        TriggerClientEvent("blackjack:notify",source,"~r~Você não tem fichas suficientes!")
                    end
                end
            end
        end
    else
        TriggerClientEvent("blackjack:notify",source,"~r~Ocorreu um erro com sua aposta!")
    end
end)
