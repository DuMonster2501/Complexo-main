...
RegisterNetEvent('blackjack:updatePlayerChips')
AddEventHandler(
            ...
            if closestChair ~= nil and closestChairDist < 1.7 then
                if not timeoutHowToBlackjack then
                    if blackjackTableData[closestChair] == false then 
                        drawNativeNotification("Pressione ~INPUT_PICKUP~ para jogar blackjack")
                    else 
...
Citizen.CreateThread(function()
    while true do 
        ...
        if drawCurrentHand then 
			DrawAdvancedNativeText(0.590, 0.935, 0.005, 0.0028, 0.40, "Sua mão é ["..tostring(currentHand).."], a mão do negociante é ["..tostring(dealersHand).."]", 255, 255, 255, 255, 0, 0)
        end
        Wait(1)
    end
end)
...
Citizen.CreateThread(function()
    while true do
        if closestChair ~= -1 and closestChairDist < 1.7 then 
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cmgblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "O objetivo do Blackjack é vencer a mão do negociante sem ultrapassar 21. Este jogo usa quatro baralhos de 52 cartas, que são embaralhados no início de cada mão.                                                                                    O negociante ficará no soft 17.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
    ...
    while true do
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                ...
                DrawAdvancedNativeText(0.147, 0.935, 0.005, 0.0028, 0.29, "VALOR DA APOSTA:", 255, 255, 255, 255, 0, 0)
                DrawAdvancedNativeText(0.215, 0.928, 0.005, 0.0028, 0.464, tostring(currentBetAmount), 255, 255, 255, 255, 0, 0)
                --DrawTimerBar2("BET",tostring(currentBetAmount),1)
            end
			if blackjackactived then
			DrawRect(0.078, 0.840, 0.140, 0.032, 0, 0, 0, 150) 
            DrawAdvancedNativeText(0.147, 0.847, 0.005, 0.0028, 0.29, "SUAS FICHAS:", 255, 255, 255, 255, 0, 0)
            DrawAdvancedNativeText(0.215, 0.840, 0.005, 0.0028, 0.464, string.format('%s', frm_GroupDigits(PlayerOwnedChips)), 255, 255, 255, 255, 0, 0)
			end
        end
        if drawTimerBar then
            DrawRect(0.060, 0.886, 0.081, 0.032, 0, 0, 0, 150)
            DrawAdvancedNativeText(0.137, 0.892, 0.005, 0.0028, 0.29, "TEMPO:", 255, 255, 255, 255, 0, 0)
            if timeLeft < 10 then
                --DrawTimerBar2("TIME","00:0" .. tostring(timeLeft),1)
                DrawAdvancedNativeText(0.173, 0.885, 0.005, 0.0028, 0.464, "00:0" .. tostring(timeLeft), 255, 255, 255, 255, 0, 0)
            else
                --DrawTimerBar2("TIME","00:" .. tostring(timeLeft),1)
                DrawAdvancedNativeText(0.173, 0.885, 0.005, 0.0028, 0.464, "00:" .. tostring(timeLeft), 255, 255, 255, 255, 0, 0)
            end
        end
...
AddEventHandler("Blackjack:beginBetsBlackjack",function(gameID,tableId)
    ...
    Citizen.CreateThread(function()
        ...
        if not bettedThisRound then
            ...
            drawNativeNotification("Nenhuma aposta feita, próxima rodada")
        end
    end)
end)
        ...
        if closestChair == chairId then
            ...
            Citizen.CreateThread(function()
                ...
                if not standOrHitThisRound and sittingAtBlackjackTable then
                    ...
                    TriggerServerEvent("Blackjack:standBlackjack",globalGameId,globalNextCardCount)
                    declineCard()
                    drawNativeNotification("Você errou o tempo, por favor fique de pé.")
                end
            end)
        else 
...
function goToBlackjackSeat(blackjackSeatID)
    ...
    drawNativeNotification("Aguardando o próximo jogo começar...")
    blackjackAnimsToLoad = {
      "anim_casino_b@amb@casino@games@blackjack@dealer",
      "anim_casino_b@amb@casino@games@shared@dealer@",
      "anim_casino_b@amb@casino@games@blackjack@player",  
...
AddEventHandler("Blackjack:blackjackPush",function(tableId)
    if closeToCasino then
        ...
        annoyedIPushed()
        canExitBlackjack = true
        drawNativeNotification("~b~Você empatou!")
        drawCurrentHand = false
        currentHand = 0
        dealersHand = 0
    end
