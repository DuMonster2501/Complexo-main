if not IsDuplicityVersion() then -- CLIENT
    Utils = {
        ...
        ShowHelpNotification = function(msg, thisFrame, beep, duration)
            AddTextEntry('pokerNotification', msg)

            if thisFrame then
                DisplayHelpTextThisFrame('pokerNotification', false)
            else
                if beep == nil then
                    beep = true
                end
                BeginTextCommandDisplayHelp('pokerNotification')
    ...
    RegisterNetEvent('poker:showNotification')
    AddEventHandler(
        'poker:showNotification',
        function(msg)
