...
AddEventHandler(
    ...
    function(tableId)
        if SharedPokers[tableId] ~= nil then
		if activePokerTable == tableId then
                    ...
                    while clientTimer ~= nil do
                        ...
                        if clientTimer ~= nil then
                            ...
                            if clientTimer < 1 then
                                clientTimer = nil
                                Utils.ShowNotification(_U('no_react'))
                    ...
                    while activePokerTable do
                        ...
                        if playerBetted then
                            ...
                            if string.len(reactiveText) > 0 then
                                Utils.ShowHelpNotification(reactiveText)
                            end
                        end
                            ...
                            if self.TimeLeft == nil or self.TimeLeft > 0 then
                                ...
                                if IsDisabledControlJustPressed(0, 176) then
                                    if currentBetInput > 0 then
                                        ...
                                        Utils.ShowNotification(_U('no_bet_input'))
                                    end
                                end
                        ...
                        if self.Active then
                            if self.TimeLeft >= 10 then
                                DrawRect(0.060, 0.786, 0.081, 0.032, 0, 0, 0, 200)
                                DrawAdvancedNativeText(0.137, 0.792, 0.005, 0.0028, 0.29, _U('remaining_time'), 255, 255, 255, 255, 0, 0)
                                DrawAdvancedNativeText(0.173, 0.785, 0.005, 0.0028, 0.464, string.format('00:%s', self.TimeLeft), 255, 255, 255, 255, 0, 0)
                            else
                                if self.TimeLeft > 0 then
                                    DrawAdvancedNativeText(0.137, 0.792, 0.005, 0.0028, 0.29, _U('remaining_time'), 255, 255, 255, 255, 0, 0)
                                    DrawRect(0.060, 0.786, 0.081, 0.032, 0, 0, 0, 200)
                                    DrawAdvancedNativeText(0.173, 0.785, 0.005, 0.0028, 0.464, string.format('00:0%s', self.TimeLeft), 255, 255, 255, 255, 0, 0)
                                else
                                    if clientTimer ~= nil then
                                        DrawAdvancedNativeText(0.137, 0.792, 0.005, 0.0028, 0.29, _U('remaining_time'), 255, 255, 255, 255, 0, 0)
                                        DrawRect(0.060, 0.786, 0.081, 0.032, 0, 0, 0, 200)
                                        if clientTimer >= 10 then
                                            DrawAdvancedNativeText(0.173, 0.785, 0.005, 0.0028, 0.464, string.format('00:%s', clientTimer), 255, 255, 255, 255, 0, 0)
                                        else
                                            DrawAdvancedNativeText(0.173, 0.785, 0.005, 0.0028, 0.464, string.format('00:0%s', clientTimer), 255, 255, 255, 255, 0, 0)
                                        end
                                    end
                                end
                            end
                        ...
                        DrawRect(0.078, 0.842, 0.145, 0.032, 0, 0, 0, 200)
                        DrawAdvancedNativeText(0.126, 0.849, 0.005, 0.0028, 0.29, _U('table_min_max'), 255, 255, 255, 255, 0, 0)
                        DrawAdvancedNativeText(0.195, 0.842, 0.005, 0.0028, 0.464, string.format('%s-%s', self.data.MinimumBet, self.data.MaximumBet), 255, 255, 255, 255, 0, 0)

                        DrawRect(0.078, 0.885, 0.145, 0.032, 0, 0, 0, 200)
                        DrawAdvancedNativeText(0.136, 0.892, 0.005, 0.0028, 0.29, _U('current_player_chips'), 255, 255, 255, 255, 0, 0)
                        DrawAdvancedNativeText(0.203, 0.885, 0.005, 0.0028, 0.464, string.format('%s', frm_GroupDigits(PlayerOwnedChips)), 255, 255, 255, 255, 0, 0)

                        DrawRect(0.078, 0.928, 0.145, 0.032, 0, 0, 0, 200)
                        DrawAdvancedNativeText(0.147, 0.935, 0.005, 0.0028, 0.29, _U('current_bet_input'), 255, 255, 255, 255, 0, 0)
                        DrawAdvancedNativeText(0.215, 0.928, 0.005, 0.0028, 0.464, string.format('%s', frm_GroupDigits(currentBetInput)), 255, 255, 255, 255, 0, 0)
                    end
                end
            )
        else
        ...
        while true do
            ...
            if not InformationPlaying and activePokerTable == nil and activeChairData == nil then
                if closeToPokers then
                                    ...
                                    for chairBone, chairId in pairs(Config.PokerChairs) do
                                        ...
                                        if chaircoords then
                                            if #(playerpos - chaircoords) < 1.5 then
                                                ...
                                                if RageUI.Visible(RMenu:Get('aquiver_poker', 'instructions')) then
                                                    RageUI.DrawContent(
                                                        ...
                                                        function()
                                                            ...
                                                            --[[RageUI.Button(
                                                                '~HUD_COLOUR_G10~Creditos',
                                                                'Agradeço que você esteja usando meu script.~n~O script foi feito por ~b~Shimmer~w~.',
                                                                {RightLabel = '©'},
                                                                true,
                                                                function(Hovered, Active, Selected)
                                                                    if (Hovered) then
                                                            ...
                                                            --[[RageUI.Button(
                                                                '~r~Reportar/Bugs',
                                                                'Chamar administrador no discord.',
                                                                {RightLabel = '✡'},
                                                                true,
                                                                function(Hovered, Active, Selected)
                                                                    if (Hovered) then
                    ...
                    if close then
                        RageUI.Visible(RMenu:Get('aquiver_poker', 'instructions'), true)
                    else
                        frm_showed = false
                        RageUI.Visible(RMenu:Get('aquiver_poker', 'instructions'), false)
    ...
    function()
        ...
        for i = 1, #Config.Tables, 1 do
            ...
            if DoesEntityExist(obj) then
                ...
                print('(Does not matter in the config) Poker table model: ' .. Config.Tables[i])
            end
        end
...
AddEventHandler('aquiverPoker:sitDown:client', function(tableId, chairId, chairCoords, chairRotation)
     if SharedPokers[tableId] ~= nil then
